...
1. **macOS**（LaunchAgent 自启部分需要；bridge 本身在 Linux 也跑，自启机制自己换）
...
6. **lark-cli** 已装并 OAuth 登录（下面 Step 0 教）
...
更安全的 API key 注入方式（不明文落盘）：

```bash
# 从 plist 里删除 ANTHROPIC_API_KEY 那对 key/string，然后：
...
重启 bridge 生效（LaunchAgent：`launchctl stop com.yourname.fcb && launchctl start com.yourname.fcb`）。

定时任务用独立 session 跑，不污染你在飞书里的交互上下文。

飞书里动态管 cron（`/cron add ...`）还没实现，目前只能改配置文件。
...
1. `launchctl stop com.yourname.fcb` 停 bridge
2. 看 log 找原因
